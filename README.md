...
- **Signal group name resets to a base64 id**: Fix implemented, pending verification — confirmed via on-device DB inspection: `reresolveConversationNames` (run on contact/profile sync) iterated *all* conversations, and for a group (no phone/profile row) the resolver returned null, which it then wrote over the group's title. The UI then fell back to `jid.take(8)+"…"` — the base64 group id. Fixed to never overwrite a name with null (honouring its own doc: "conversations with no phone are left alone"); covered by a Robolectric DB test.
- **WhatsApp empty/ghost conversations (raw-JID names)**: Fix implemented, pending verification — on-device inspection found ~117 conversation rows with no messages and blank names (history sync creates a row per chat in WhatsApp's payload, but messages outside the retention window are never inserted). The conversation list now only shows conversations that actually have a message, dropping the ghosts (existing and future) without a migration.
- **Signal group messages from others not received**: Under investigation — confirmed via DB (a group with 3 sent / 0 received) and logcat (only `hasSync`/`hasReceipt` envelopes from others, zero `hasData`, zero decrypt failures). Sent messages work; inbound group content isn't arriving as data at all. Leading hypothesis: linked-device sender-key delivery — other members' clients can't establish a session with this linked device to deliver their sender keys (possibly tied to the known-fragile pre-key upload path), so they never send group ciphertext here. DMs from existing sessions still arrive. Needs protocol-level investigation.
...
- **Signal reaction send not implemented**: Known limitation — `SignalRepository.sendReaction` is a stub, so the Signal chat screen no longer shows the reaction picker (it was a dead control). WhatsApp reactions are unaffected.
...
- **WhatsApp group detection heuristic + sticky `is_group`**: history sync falls back to a participant-count heuristic when the JID server check fails, and `UpsertConversation` makes `is_group` sticky-on-true (never cleared), so a single misclassification is permanent. Fix: classify solely by JID server (`g.us`).
- **History `last_message_id` taken from the pre-filter tail**: history sync derives a conversation's last message from the raw `msgs` tail, which may have been skipped by retention/nil filtering and never inserted — so the conversation preview can point at a non-existent row. Fix: use the last *inserted* row.
