...
- **Signal media send (+ button missing)**: Photo retry succeeded on device in v0.3.2-rc.2 after the attachment uploader switched to a REST upload-form request. The attach button is wired through `onSendMedia` to `SignalRepository.sendMedia`; video delivery remains untested on device.
...
- **Signal reaction send**: Implemented, pending device verification — long-press now offers the shared emoji picker (including 🤙), sends a Signal reaction to the target message in DMs and groups, and shows a local reaction after the send succeeds.
...
- **Voice notes**: Recording and send controls are wired for both platforms. WhatsApp and Signal sends remain untested on device.
