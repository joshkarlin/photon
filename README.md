...
- **Signal send results are not fully checked**: The DM send and self-device sent-transcript paths ignore the library's returned `SendMessageResult.isSuccess` flag. A non-throwing failure could therefore leave a message marked sent locally or omit it from another linked device. The rc.1 session fix resolved the observed send/sync failure on device, but this separate result-handling gap remains.
...
- **WhatsApp group switches to DM view when you send a message**: Fix implemented, pending verification — sending into a group flipped the chat from the group (Transcript) layout to the DM (Terminal) layout. The send/confirm path bumps the conversation preview with a nameless `UpsertConversation(jid, "", false, …)`, and the recently-added trusted `is_group = excluded.is_group` overwrite honoured that hardcoded `false`, clobbering the group's `is_group=1` → `0`; the chat screen then picked the DM layout. Fixes: the send, media-send and send-confirm paths derive `is_group` from the target JID (`@g.us`) instead of hardcoding `false`; and `UpsertConversation` now treats a *nameless* upsert as a preview bump only — it never reclassifies `is_group` (authoritative classification always carries a name, and `RepairConversationMetadata` still fixes stale rows by JID). Covered by a Go test that a nameless bump on a group keeps `is_group=1`.
...
- **Signal group messages from others not received**: Fix implemented, pending verification — confirmed via DB (a group with 3 sent / 0 received) and logcat (only `hasSync`/`hasReceipt` envelopes from others, zero `hasData`, zero decrypt failures). Sent messages work; inbound group content wasn't arriving as data at all. First fix: linked-device session priming. Photon already sent silent NullMessage session pings for DM contacts; this now expands to GroupV2 participants as well, so group-only members learn Photon's linked `deviceId` and include it in future encrypted recipient lists. Reconnect also refreshes GroupV2 metadata for groups with an empty participants table so there are member ACIs to ping. Follow-up rc20 investigation (Jun 23 2026) showed the ping job running (`8` known ACI candidates, `2` due, both stale/unregistered `404`) but group messages were still absent from the receive queue during the captured window. Second fix: receiver now handles top-level `senderKeyDistributionMessage` content and stores the sender-key session instead of acking it as an unhandled envelope; without that key material, later sender-key group ciphertext from that member cannot decrypt. Protocol decrypt failures now send Signal retry receipts for retryable missing/invalid session-key cases, giving the sender a chance to resend the missing group key material instead of Photon silently acking the failure. Added explicit sender-key-distribution and retry-receipt logging. On-device capture (Jun 24 2026) finally caught the real failure: an inbound sealed group sender-key envelope failing with `ProtocolNoSessionException: missing sender key state for distribution ID …` — confirming the data *does* arrive and the retry-receipt path is the correct recovery. Third fix: the retry receipt was built with the wrong device id — `DecryptionErrorMessage.forOriginalMessage(...)` was passed our own linked-device id instead of the `originalSenderDeviceId` (the remote member's sending device). The sender keys its Message Send Log lookup on its own sending device id + timestamp, so our malformed receipt missed and it never re-distributed the sender key (silent no-op). Now passes the sender's device id. Needs on-device confirmation that, after a failed decrypt, the sender re-sends its SKDM and subsequent group messages from that member decrypt. Update (rc24): a group message decrypted successfully — but appeared **36 times**. Cause: incoming messages were keyed by `{author}_{timestampMs}_{random}`, and the random suffix meant every redelivery (sender resending after our retry receipt, plus server re-delivery of un-acked envelopes) minted a fresh id and stored a duplicate row. Fix: dedupe incoming + sent-transcript inserts on the stable `{author}_{timestampMs}` prefix (skip if already stored), and a one-time startup cleanup collapses the existing duplicate rows. Covered by a Robolectric test.
...
- **Signal media send**: Still broken in v0.3.2-rc.1. Supplying the required resumable-upload spec fixed the original `NoSuchElementException`, but the library's upload-form API then failed because Photon's WebSocket wrapper does not support chat connection access. A REST upload-form request plus the library's spec builder is implemented, pending device verification. This shared path covers images, video, voice notes, and media retry.
...
- **Signal media send (+ button missing)**: The attach button is wired through `onSendMedia` to `SignalRepository.sendMedia`; photo/video delivery still needs on-device verification after the upload fix above.
...
- **Signal read receipts from other devices not received**: Fix implemented, pending on-device verification — when a message is read on the primary device or another linked device, Photon now matches the incoming `sync.read` entry to the stored author/timestamp message, updates its read state, and clears only the affected conversation's unread count and notification. Group messages resolve through their stored conversation JID. This is separate from Photon sending its own self-device read syncs.
- **Signal primary-device transcripts could leave Photon unread**: Fix implemented, pending on-device verification — after a message was sent on the primary, Photon could receive the `sync.sent` transcript without receiving the corresponding `sync.read`. The outgoing message was stored correctly, but any existing incoming unread count remained and the chat still appeared unread. Processing a valid primary-device sent transcript now marks the destination conversation's incoming messages read locally as a fallback; the normal `sync.read` path remains in place when it arrives.
- **Signal receiver connection flapped during sync/send work**: Fix implemented, pending on-device verification — logcat showed the authenticated `photon-recv` WebSocket connect and then close shortly after the sender opened its own authenticated `photon-send` socket, with no decrypted envelopes observed. Photon now shares the receiver-owned authenticated and unauthenticated sockets with the sender, avoiding two authenticated connections for the same linked device. Sender invalidation no longer disconnects sockets owned by the receiver.
...
- **Voice notes**: Recording and send controls are wired for both platforms. WhatsApp send remains untested on device; Signal send needs on-device verification after the media-upload fix above.
...
Photon will remain fully on-device; a developer-operated messaging relay is out of scope. Distribution through the Light SDK depends on Light supporting individually approved real-time/background capabilities and vetted protocol dependencies. See [lightphone/light-sdk#128](https://github.com/lightphone/light-sdk/issues/128) for the current discussion.

- [ ] Establish an on-device reliability baseline for each protocol: idle battery drain, memory use, socket uptime, reconnect frequency and backoff, and message latency over a representative 24-48 hour run
- [ ] Test and document lifecycle behavior for app foreground/background, screen lock, Wi-Fi/cellular transitions, airplane mode, process death, and reboot, with Signal and WhatsApp results recorded separately
- [ ] Add explicit per-protocol background-operation controls and status, including one action that closes connections and prevents automatic restart
- [ ] Publish a dependency inventory covering source, license, pinned version or commit, native artifacts and ABIs, update procedure, and reproducible build path
- [ ] Document protocol-specific distribution risks and support boundaries separately for WhatsApp, Signal, and SMS
- [ ] State clearly in the README and release notes that Photon is community-supported and that Light does not own protocol outages or compatibility failures
- [ ] Prototype Light SDK UI and navigation integration independently of protocol and background-service approval
- [ ] If Light exposes restricted real-time APIs, build a no-cloud SDK compatibility target and validate it against the lifecycle and reliability baseline above
- [ ] If Light accepts third-party protocol dependencies, request precise allowlist changes for Turasa `signal-service-java`, Signal's `libsignal-android`, and a vetted, reproducible Go/whatsmeow native artifact
