...
- **WhatsApp group message sender names show as numbers**: Under investigation — after re-pairing, per-message sender labels in group transcripts render the phone number instead of the contact/profile name, even though the conversation list shows correct names. Indicates the `participants` table lacks display names for the message senders' `sender_jid` keys (or the keys don't line up), while the DM/conversation-name path (which resolves fine) takes a different route. Needs on-device DB inspection to confirm whether it's an unresolved-LID key split, an empty contact store at sync time, or a backfill timing issue.
