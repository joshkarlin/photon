...
- **Signal media send**: Fix implemented, pending device verification. An image send failed before CDN upload because the attachment stream lacked the required resumable-upload spec. Photon now requests a spec using the padded ciphertext length and attaches it to the stream. This shared path covers images, video, voice notes, and media retry; verify delivery and retry on device.
