...
- **Signal linked-device sync or Note to Self send fails after a session becomes unusable**: Fix implemented, pending on-device verification. On Sep 23, device logs showed `InvalidSessionException: Session not found` while encrypting to another device on the same account; read sync failed for the same device. A recipient can receive a DM while its separate sent transcript fails to reach the primary device. The session store counted a saved row as active even when its record had no sender chain, so the Signal sender failed before it could reach its normal device-list mismatch and pre-key refresh path. The store now advertises only encryptable sessions. Existing failed messages are not resent automatically.
